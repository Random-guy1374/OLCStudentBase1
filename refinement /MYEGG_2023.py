...
for i in range(len(egg_code)):
    check = 0
    if len(egg_code[i]) > 5: #7 change 7 to 5
        check += 1
    if egg_code[i][0] in "0123": #1) change [0123] to "0123"
        check += 1
    if egg_code[i][1:3].isalpha(): #2) change .isalpha to .isalpha()
        check += 1
    if egg_code[i][3:].isdigit(): #3) change .isdigit to .isdigit()
        check += 1
    if check == 4: #8) change 3 to 4
        valid += 1 #4) change valid = 1 to valid += 1
...
if valid == len(egg_code):
    ...
    farm_method_eggs = [0,0,0,0]
    for j in range(len(egg_code)):
        farm_method_eggs[int(egg_code[j][0])] += 1 #9 add int()
    farm_method = ['Organic','Free Range','Barn','Cage']
    for k in range(len(farm_method)): #14) change egg_code to farm_method
        print("Number of {0} eggs: {1}".format(farm_method[k],farm_method_eggs[k]))
    # Collate the number of eggs sampled according to country of origin
    countries = ['UK', 'FR', 'NL']
    countries_eggs = [0,0,0]

    for m in range(len(countries)): #10) change egg_code to countries
        for n in range(len(egg_code)): #11) change countries to egg_code
            if egg_code[n][1:3] == countries[m]: #12 change [m][:2] to [n][1:3] 13) change countries[n] to countries[m]
                countries_eggs[m] += 1 #6) tab
    for p in range(len(countries_eggs)):
        print("Number of {0} eggs: {1}".format(countries[p], countries_eggs[p]))
else: #5) add :
    print("Invalid egg codes found for this batch of eggs.")